...
Смотри, теперь давай обработаем лимиты магазина для загрузки. Перед каждой загрузкой товаров в магазин, наш сервер должен отправлять запрос к API озона для того чтобы понять можно ли загрузить в этот магазин карточки.
Информация о лимитах должна показываться также в настройках под каждым магазином
Метод:
