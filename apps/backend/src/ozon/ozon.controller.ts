...
export class OzonController {
  ...
  @Roles(UserRole.ADMIN)
  @ApiOperation({
    summary: "Опубликовать несколько карточек на Ozon (батчами по 100)",
  })
  ...
  @Roles(UserRole.ADMIN)
  @ApiOperation({
    summary: "Получить все незаимпортированные product_id одним запросом",
  })
  ...
  listImportableProducts(
    ...
    return this.ozonService.listImportableProducts(
      storeId,
      page,
      limit,
      visibility,
  ...
  importProducts(
    ...
    return this.ozonService.importProducts(
      dto.productIds,
      user.id,
      dto.storeId,
      dto.isArchived,
  ...
  @Roles(UserRole.ADMIN)
  @ApiOperation({
    summary: "Привязать магазины к ранее опубликованным карточкам",
  })
  backfillStoreIds() {
  ...
  @Roles(UserRole.ADMIN)
  @ApiOperation({
    summary: 'Сбросить книги, зависшие в статусе "Публикуется в Ozon" > 24ч',
  })
  ...
  @Roles(UserRole.ADMIN)
  @ApiOperation({
    summary: "Синхронизировать статус архивации товаров на Ozon",
  })
  syncArchivedStatus() {
    this.ozonService
      .syncArchivedStatus()
      .catch((e) =>
        this.ozonApiClient["logger"]?.error(
          "syncArchivedStatus background error",
          e,
        ),
      );
  ...
  @Roles(UserRole.ADMIN)
  @ApiOperation({
    summary: "DEBUG: сырой ответ словаря Ozon (первые 10 значений)",
  })
  @ApiQuery({ name: "attr", description: "brand | author", required: false })
  @ApiQuery({
    name: "value",
    description: "Фильтр value (опционально)",
    required: false,
  })
